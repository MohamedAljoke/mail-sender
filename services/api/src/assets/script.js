...
function handleJobCreated(jobData) {
    ...
    const existingJobIndex = allJobs.findIndex(job => job.job_id === jobData.job_id);
...
function showStatusNotification(jobData) {
    ...
    const statusIcon = jobData.status === 'completed' ? '✅' : 
                      jobData.status === 'processing' ? '⚙️' : 
...
async function handleEmailSubmit(event) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            resultDiv.innerHTML = `<strong>❌ Error:</strong> ${result.error || 'Failed to send email'}`;
        }
...
async function loadCompleteJobHistory() {
    try {
        ...
        if (response.ok && data.jobs) {
            ...
            allJobs.sort((a, b) => new Date(b.created_at || b.timestamp) - new Date(a.created_at || a.timestamp));
...
function displayJobHistory() {
    ...
    if (allJobs.length === 0) {
        container.innerHTML = '<p class="no-jobs">No jobs yet. Submit an email to see live updates!</p>';
        return;
    }
...
function createJobCard(job) {
    ...
    if (job.history && Array.isArray(job.history) && job.history.length > 0) {
        const sortedHistory = job.history.sort((a, b) => new Date(a.timestamp) - new Date(b.timestamp));
        historyHTML = `
                ...
                <div class="history-timeline">
                    ${sortedHistory.map(entry => `
                        <div class="history-item ${entry.status}">
                            ...
                            ${entry.error ? `<div class="error-message">Error: ${entry.error}</div>` : ''}
                        </div>
                    `).join('')}
                </div>
            </div>
        ...
        <div class="job-card status-${job.status}">
            ...
            <div class="job-details">
                ...
                <div class="job-detail-row">
                    ...
                    <span>${timeAgo} (${formatTimestamp(job.created_at || job.timestamp)})</span>
                </div>
                <div class="job-detail-row">
                    <strong>Updated:</strong>
                    <span>${formatTimestamp(job.updated_at || job.timestamp)}</span>
                </div>
                ${job.retry_count ? `
                    <div class="job-detail-row">
...
function clearJobHistory() {
    if (confirm('Are you sure you want to clear the local job history? This will only clear locally stored data.')) {
        allJobs = [];
